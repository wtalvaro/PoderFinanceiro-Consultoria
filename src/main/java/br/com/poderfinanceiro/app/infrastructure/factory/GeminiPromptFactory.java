...
public class GeminiPromptFactory {
    ...
    public String getRecomendacaoEstrategicaPrompt(SimulacaoRascunhoDTO perfil, List<ResultadoSimulacaoDTO> opcoes) {
        ...
        for (int i = 0; i < opcoes.size(); i++) {
            ...
            sb.append(i + 1).append(". Banco: ").append(op.tabela().getBanco().getNome()).append(" | Tabela: ")
                    .append(op.tabela().getNomeTabela()).append(" | Taxa de Juros Mensal: ")
                    .append(op.tabela().getTaxaMensal()).append("%").append(" | Comissão Percentual Base: ")
                    .append(op.tabela().getComissaoPercentual()).append("%").append(" | Parcela Estimada: R$ ")
                    .append(op.valorParcela()).append("\n");
        }
